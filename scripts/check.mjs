...
const requiredFiles = ['index.html', 'styles.css', 'app.js', 'data.json', 'events.json', 'group-history.json', 'banner_3.png', 'favicon.png', 'operator.jpg', 'institution-light.png', 'institution-dark.png', 'CNAME', '.nojekyll'];
...
const groupHistory = JSON.parse(await readFile(join(root, 'docs', 'group-history.json'), 'utf8'));
const slugs = new Set();
...
console.log(`检查通过：${data.entries.length + groupHistory.entries.length} 篇专题（含 ${groupHistory.entries.length} 篇匿名化群史），${eventData.events.length} 条官网事件，GitHub Pages 配置完整。`);
